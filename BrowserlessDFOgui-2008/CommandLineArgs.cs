...
namespace Dfo.BrowserlessDfoGui
{
	class CommandLineArgs
	{
		...
		public string CustomSoundpackDir { get; private set; } // give default after processing
		public string TempSoundpackDir { get; private set; } // give default after processing
		...
		public OptionSet GetOptionSet()
		{
			OptionSet optionSet = new OptionSet()
			{
				...
				{ "u|username=", "Username to use when logging in.", argValue => Username = argValue },
				{ "pw|password=", "Password to use when logging in.", argValue => Password = argValue },
				{ "closepopup", "Close the popup when the game is done. This is the default.", argExistence => ClosePopup = (argExistence != null) },
				{ "noclosepopup", "Don't close the popup when the game is done.", argExistence => ClosePopup = !(argExistence != null) },
				{ "windowed", "Launch the game in windowed mode.", argExistence => LaunchWindowed = (argExistence != null) },
				{ "full", "Don't launch the game in windowed mode. This is the default.", argExistence => LaunchWindowed = !(argExistence != null) },
				{ "soundswitch", "Switch soundpacks.", argExistence => SwitchSoundpacks = (argExistence != null) },
				{ "nosoundswitch", "Don't switch soundpacks. This is the default.", argExistence => SwitchSoundpacks = !(argExistence != null) },
				{ "dfodir=", "Directory where DFO is. Defaults to the directory this program is in.", argValue => DfoDir = argValue },
				{ "customsounddir=", "Directory where custom soundpacks are if switching soundpacks. Defaults to dfodir/SoundPacksCustom.", argValue => CustomSoundpackDir = argValue },
				{ "tempsounddir=", "Directory to rename the normal soundpack directory while the game is running if switching soundpacks. Defaults to dfodir/SoundPacksOriginal.", argValue => TempSoundpackDir = argValue },
		...
		public override string ToString()
		{
			...
			builder.AppendLine( string.Format( "Username specified = {0}", Username != null ) ); // Don't show username for security reasons
			builder.AppendLine( string.Format( "Password specified = {0}", Password != null ) ); // Don't show password for security reasons
			builder.AppendLine( string.Format( "Close popup = {0}", ClosePopup ) );
			builder.AppendLine( string.Format( "Launch windowed = {0}", LaunchWindowed ) );
			...
			builder.Append( string.Format("Temp soundpack dir = {0}", TempSoundpackDir ));
